...
// Importuj zaktualizowany interfejs Initiative
...
export type SortConfig = {
    key: keyof Initiative | null; // Klucz może być teraz jednym z nowych pól
...
const InitiativeTable: React.FC<InitiativeTableProps> = ({
    ...
    // Zaktualizowana definicja kolumn
    const columns: { key: keyof Initiative | 'actions'; label: string; visible: string; sortable: boolean }[] = [
        ...
        { key: 'entity_status_display', label: 'Statut Podmiotu', visible: 'hidden lg:table-cell', sortable: true }, // Sortujemy po kluczu, wyświetlamy display
        { key: 'implementation_area_display', label: 'Obszar', visible: 'hidden sm:table-cell', sortable: true }, // Sortujemy po kluczu, wyświetlamy display
        { key: 'location_text', label: 'Miejsce', visible: 'hidden xl:table-cell', sortable: true },
        { key: 'funding_source_display', label: 'Finansowanie', visible: 'hidden md:table-cell', sortable: true }, // Sortujemy po kluczu, wyświetlamy display
                    ...
                    <tr>
                        {columns.map(col => (
                            <th key={col.key} scope="col" className={`px-6 py-3 ${col.visible}`}>
                                {col.sortable ? (
                                    // Używamy klucza display do etykiety, ale klucza danych do sortowania
                                    <button
                                        type="button"
                                        // Do sortowania używamy klucza bez '_display', jeśli istnieje
                                ...
                                if (col.key === 'actions') {
                                    return (
                                        <td key={`${initiative.id}-actions`} className={`px-6 py-4 ${col.visible} space-x-3 whitespace-nowrap`}>
                                            <button onClick={() => onEdit(initiative)} className="..."> <PencilSquareIcon className="h-4 w-4 mr-1" /> Edytuj </button>
                                            <button onClick={() => onDelete(initiative)} className="..."> <TrashIcon className="h-4 w-4 mr-1" /> Usuń </button>
                                        </td>
                                    );
                                }

                                // Wyświetl odpowiednią wartość - używamy pól *_display dla czytelności
